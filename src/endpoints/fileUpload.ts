...
export class FileUpload extends OpenAPIRoute {
    ...
    async handle(c: Context) {
        ...
        await c.env.CLEANUP_QUEUE.send(
            ...
            {
                delaySeconds: Math.min(data.body.expiry, config.queue.maxRetryDelay),
                contentType: 'json'
            }
        );
